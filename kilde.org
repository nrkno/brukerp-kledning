...
- Er brukeren pålogget flere tjenester?
...
#+begin_src python
  ...
  def hent_ut_data(df):
       df.loc[:,["data"]] = df.hop.apply(lambda x: eval(x))
       return df
#+end_src
...
Nå ser vi på noen statistikker for hopene med mer enn én bruker.
#+begin_src python :results replace
...
: count  104872.000000  104872.000000  104872.000000
: mean        1.308166       2.725780       2.363262
: std         0.537259       1.081599       0.646862
...
#+end_example
**** Utvidede klynger
Nå skal jeg ta dette et steg videre, og basere klyngene på alle typer enheter og tjenester.
...
#+begin_src bigquery
  ...
  WITH
    TVBRUKERPROFIL AS (
      /* Her er alle brukere som har brukt NRK TV på en TV-klient de siste
         30 dagene.
      */
      SELECT DISTINCT visitorId, nrkUserId, userOrProfile
        FROM `nrk-datahub.snowplow_processed.views_v02`
    ...
    UTVIDET_TVBRUKERPROFIL AS (
      /* Her er de andre enhetene og tjenestene brukerne over har brukt
         de siste 30 dagene.
      */
      SELECT DISTINCT v.visitorId, nrkUserId, nrkService, platform, userOrProfile,
        FROM `nrk-datahub.snowplow_processed.views_v02` v
             JOIN TVBRUKERPROFIL USING(nrkUserId)
       WHERE partitionDate > CURRENT_DATE - 7),
    
    ALLE_BRUKERPROFILER AS (
      /* Her er alle andre brukere. Disse skal kobles sammen med brukerne over
         gjennom enhetene som brukerne over har brukt.
      */
      SELECT DISTINCT visitorId, nrkUserId, userOrProfile, nrkService, platform,
        FROM `nrk-datahub.snowplow_processed.views_v02`
             JOIN `nrk-datahub.prod.registered_users_v01` USING(nrkUserId)
       WHERE partitionDate > CURRENT_DATE - 7),

    BRUKER_BRUKER_KOBLING AS (
      /* Her kobles brukerne fra UTVIDET_TVBRUKERPROFIL sammen med brukerne fra
         ALLE_BRUKERPROFILER sammen gjennom enhetene som de har brukt.
      */
      SELECT DISTINCT bp1.nrkUserId, bp1.visitorId, bp1.platform, bp1.nrkService, bp2.nrkUserId koblet_til, bp2.userOrProfile,
        FROM UTVIDET_TVBRUKERPROFIL bp1
             JOIN ALLE_BRUKERPROFILER bp2 USING(visitorId)
       WHERE bp1.userOrProfile = 'user'),

    BRUKERKLYNGER AS (
      /* Her konkatenerer jeg sammen alle brukerne som har vært logget på samme klient.
      */
      SELECT nrkUserId, visitorId, platform, nrkService,
             CONCAT('["', STRING_AGG(koblet_til, '","' ORDER BY koblet_til), '"]') klynge,
        FROM BRUKER_BRUKER_KOBLING
     GROUP BY ALL),

    BRUKERKLYNGER_KONKAT AS (
      /* Her legger jeg inn platform og tjeneste sammen med klyngen i en streng*/
      SELECT nrkUserId, visitorId,
             CONCAT('{"platform": "', platform, '", "tjeneste": "', nrkService, '", "klynge": ', klynge, '}') klynge
        FROM BRUKERKLYNGER),
    
    BRUKERHOPER AS (
      /* Til slutt samler jeg sammen klyngene i hoper ved å koble sammen alle klyngene
         som en bruker er med i.*/
      SELECT bk1.klynge brukere, 
             CONCAT('{"id": ', ROW_NUMBER() OVER(),', "data": [', STRING_AGG(DISTINCT bk2.klynge, ', ' ORDER BY bk2.klynge), ']}') hop
        FROM BRUKERKLYNGER_KONKAT bk1
             JOIN BRUKERKLYNGER_KONKAT bk2 USING(nrkUserId)
       GROUP BY ALL)
